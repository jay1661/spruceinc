 ...
 * @extends WC_Widget
 */
class Carafity_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'carafity');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'carafity');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'carafity'),
				'label' => esc_html__('Title', 'carafity'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'carafity'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'carafity'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'carafity'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'carafity'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'carafity'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'carafity'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'carafity'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Carafity_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'carafity');
			...
			echo '<ul class="product-brands" id="carafity-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Carafity_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'carafity'),
		'singular_name'              => esc_html__('Brands', 'carafity'),
		'menu_name'                  => esc_html__('Brands', 'carafity'),
		'all_items'                  => esc_html__('All Brands', 'carafity'),
		'parent_item'                => esc_html__('Parent Brand', 'carafity'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'carafity'),
		'new_item_name'              => esc_html__('New Brand Name', 'carafity'),
		'add_new_item'               => esc_html__('Add New Brands', 'carafity'),
		'edit_item'                  => esc_html__('Edit Brand', 'carafity'),
		'update_item'                => esc_html__('Update Brand', 'carafity'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'carafity'),
		'search_items'               => esc_html__('Search Brands', 'carafity'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'carafity'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'carafity'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
