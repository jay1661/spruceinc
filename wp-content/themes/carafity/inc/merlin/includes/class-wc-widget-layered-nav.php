...
class Carafity_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce carafity_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Carafity custom)', 'carafity');
        $this->widget_id          = 'carafity_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Carafity Filter Products by Attribute', 'carafity');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'carafity'),
                'label' => esc_html__('Title', 'carafity'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'carafity'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'carafity'),
                'options' => array(
                    'and' => esc_html__('AND', 'carafity'),
                    'or'  => esc_html__('OR', 'carafity'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="carafity-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($attribute_type)) {
                        ...
                        switch ($attribute_type) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="carafity-color-type" href="' . esc_url($link) . '">';
                                $term_html .= '<span class="color-label"><span style="background: ' . $color . ';"></span></span>';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="carafity-image-type" href="' . esc_url($link) . '">';
